...
def compute_mfcc(audio_data, sample_rate):
    ...
    for i in range(NUM_CEPSTRUM):
        for j in range(NUM_FILTERS):
            ESP32_MFCC[i] += log_mel_spectrum[j] * math.cos(math.pi * i / NUM_FILTERS * (j + 0.5))
...
def main():
    ...
    #setup for continuous audio sampling 
...
# Run the main function; Continuously computes MFCCs through continuous audio sampling
while True:
    main()  
...
def sync():
    ...
    client.connect(('192.168.26.174', 1234))  # Replace with ESP32-M's IP address
